...
addMatchToUserList = (user, match, operation, next) ->
  if operation == true
    User.update { _id: user.id }, { $addToSet: { 'matches': match } },
    (err, numAffected) ->
      if err
        return { message: err }
      else
    ...
    User.findByIdAndUpdate { _id: user.id }, { $pull: 'matches': match },
    (err, numAffected) ->
      if err
        return { message: err }
      else
      ...
      List.findOne { '_id': list_id }, (err, list) ->
        ...
        if diffMinutes > -360
          ...
          'names': $elemMatch: 'player_id': player_id,(err, userFound) ->
            ...
            is_there_waiting_list =
            if users_playing >= list.list_size then true else false
